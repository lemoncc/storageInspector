...
| 区域 | 功能 |
|------|------|
| 顶栏 | 筛选 Key/值（自绘下拉：最近 5 次筛选 + 当前 Key，可清除；默认空）；刷新；JSON 查看 / JSON 编辑；导出 / 导入；清空 |
...
导出为 JSON v2；Cookie 额外带 `cookies[]`（完整详情，同名多 Path 不合并）。`data{}` 仅为兼容折叠（同名只保留第一条），导入请优先用 `cookies[]`。
